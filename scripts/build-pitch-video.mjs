 ...
 *   ffmpeg -f concat -safe 0 -i /tmp/vid/concat.txt -i /tmp/vid/voice.wav ...
 *
 * Frames are skipped when already rendered, so an interrupted run resumes.
 * Voices come from `say -v '?'`; re-running with a different VOICE only
 * regenerates audio when the frames are already on disk.
 */
