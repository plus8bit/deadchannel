...
const server = new McpServer({
  ...
  version: "0.1.0",
  instructions:
    "Company facts from a domain, paid per call in USDC on Base. Four tools at four prices, so a " +
    "cheap question does not pay for an expensive answer: hosaka_lookup is a $0.01 summary; " +
    "hosaka_dossier ($0.07) returns every third-party vendor the company can be proven to use, each " +
    "with the DNS record or script that proves it; hosaka_contacts ($0.02) adds the addresses and " +
    "phone numbers the company publishes about itself; hosaka_people ($0.25) adds named individuals " +
    "who work there. Requires HOSAKA_PRIVATE_KEY, a wallet holding a little USDC.",
});
...
server.tool(
  {
    ...
    description:
      ...
      "use hosaka_people instead. Costs $0.02 in USDC.",
    inputSchema: DOMAIN_SCHEMA,
  },
  async (args) => {
    try {
...
server.tool(
  {
    ...
    description:
      "Everything hosaka_dossier returns, plus the people who decide: owners, founders, C-level, " +
      "partners, VPs, heads and directors, each with title, location and profile link. Use this " +
      "when the question is who to approach about buying something. Use hosaka_people instead to " +
      "see anyone who works there regardless of level. Costs $0.30 in USDC.",
    inputSchema: DOMAIN_SCHEMA,
  },
  async (args) => {
    try {
...
server.tool(
  {
    ...
    description:
      "Everything hosaka_dossier returns, plus named people who work at the company, sourced from a " +
      "people-data provider. Cheaper than buying the contacts alone elsewhere. Use hosaka_contacts " +
      "instead when a published support or sales address would answer the question. Costs $0.25 in USDC.",
    inputSchema: DOMAIN_SCHEMA,
  },
  async (args) => {
    try {
