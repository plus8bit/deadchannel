...
server.tool(
  {
    ...
    description:
      ...
      `see anyone who works there regardless of level. Costs ${PRICES.executives} in USDC.`,
    inputSchema: DOMAIN_SCHEMA,
  },
  async (args) => {
    try {
...
server.tool(
  {
    ...
    description:
      ...
      `instead when a published support or sales address would answer the question. Costs ${PRICES.people} in USDC.`,
    inputSchema: DOMAIN_SCHEMA,
  },
  async (args) => {
    try {
